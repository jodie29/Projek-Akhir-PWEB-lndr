...
if(!$order) { echo "No suitable Bayar Nanti order found to test approve\n"; exit; }

echo "Approving order {$order->id} without skip_confirmation as admin {$admin->id}\n";
$controller = new OrderApprovalController();
$request = Request::create('/admin/orders/'.$order->id.'/approve', 'POST', ['actual_weight' => 2]);
