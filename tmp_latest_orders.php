...
$stmt=$pdo->query("SELECT id,order_number,status,created_at FROM orders WHERE status NOT IN ('awaiting_confirmation') ORDER BY created_at DESC LIMIT 10");
foreach($stmt->fetchAll(PDO::FETCH_ASSOC) as $r) echo $r['id'].' | '.$r['order_number'].' | status:'.$r['status'].' | '.$r['created_at']."\n";
