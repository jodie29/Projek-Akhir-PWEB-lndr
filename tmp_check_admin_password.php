...
$stmt = $pdo->prepare("SELECT password FROM users WHERE email = 'admin@powerwash.test'");
$stmt->execute();
$row = $stmt->fetch(PDO::FETCH_ASSOC);
if(!$row){ echo "Admin user not found"; exit(1); }
$hash = $row['password'];
$test = 'password';
if(password_verify($test, $hash)){
    echo "Password cocok: $test\n";
}else{
    echo "Password TIDAK cocok: $test\n";
}
